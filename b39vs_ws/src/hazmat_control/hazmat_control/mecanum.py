...
class Mecanum(Node):
    ...
    def twistCallback(self, msg: Twist):
        ...
        wheelCmd.front_left = int((velocity_diff - angular_term) * self.WHEEL_R_INV * 0.705)
        wheelCmd.front_right = int((velocity_result + angular_term) * self.WHEEL_R_INV * 0.9)
        wheelCmd.rear_left = int((velocity_result - angular_term) * self.WHEEL_R_INV * 0.9)
        wheelCmd.rear_right = int((velocity_diff + angular_term) * self.WHEEL_R_INV * 0.9)
        # if (msg.linear.x > 0):
