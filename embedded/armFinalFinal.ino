...
#define SERVO3_MIN 120   // Servo 2 range (0-270°)
#define SERVO3_MAX 470  
...
void loop() {
    if (Serial.available()) {
        ...
        if (input.length() > 0) {
            ...
            if (result == 4) { // Ensure four numbers are received
                if (angle1 >= 0 && angle1 <= 360 && angle2 >= 0 && angle2 <= 270 &&
                    angle3 >= 0 && angle3 <= 270 && angle4 >= 65 && angle4 <= 234) {
                    
                    int pulse1 = map(angle1, 0, 270, SERVO1_MIN, SERVO1_MAX);
                    int pulse2 = map(angle2, 0, 270, SERVO2_MIN, SERVO2_MAX);
                    int pulse3 = map(angle3, 0, 270, SERVO3_MIN, SERVO3_MAX);
    ...
    int mappedanal1 = map(analogValue1, 713, 1465, 50, 130);
    int mappedanal2 = map(analogValue2, 243, 542, 0, 200);
    int mappedanal3 = map(analogValue3, 343, 3006, 0, 180);
    int mappedanal4 = map(analogValue4, 243, 507, 0, 200);
